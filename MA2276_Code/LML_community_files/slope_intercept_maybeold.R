...
slope_matrix %>%
  group_by(YEAR) %>% 
  summarize(mean_slope = mean(slope, na.rm = T)) %>% 
  ggplot(aes(x = YEAR, y = mean_slope)) +
  geom_point() + 
  theme(axis.text.x = element_text(angle = 90)) + 
  xlim(1996, 2021) + 
  geom_smooth(method = "lm")

## Making it so it doen't consider species
...
species = species %>% rename("slope" = V1, "intercept" = V2)
